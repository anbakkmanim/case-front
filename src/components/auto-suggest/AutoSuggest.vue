    ...
    >
      ...
      >
        ...
        <feather-icon
          ...
          @click.stop="$emit('actionClicked', filteredData[currentSelected])"
        ></feather-icon>
      </li>
    </ul>
  </div>
  ...
  watch: {
    ...
    searchQuery(val) {
      if (val === '') {
        ...
        const exactEle = this.data.data.filter(item => item.label.toLowerCase().startsWith(this.searchQuery.toLowerCase()));
        const containEle = this.data.data.filter(
          item => !item.label
            .toLowerCase()
